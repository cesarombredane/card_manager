...
<template>
  <q-page class="bg-dark text-white q-pa-md q-pa-lg-md">
    ...
    <div v-if="data" class="column q-gutter-lg">
      ...
      <result-list :items="concepts"><template #default="{ item }"><router-link class="text-primary" :to="`/card-concepts/${item.id}`">{{ item.canonical_name }}</router-link><div class="text-caption text-grey-5">{{ item.card_category }}</div></template></result-list>
    </div>
  </q-page>
</template>
