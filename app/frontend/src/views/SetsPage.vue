<template>
  <q-page class="bg-dark text-white q-pa-md q-pa-lg-md">
    <div class="column q-gutter-lg">
      <page-header title="Sets" eyebrow="Browse" description="Language and region specific Pokémon TCG sets." />
      ...
      <result-list :items="results">
        <template #default="{ item }">
          ...
          <div class="text-caption text-grey-5">{{ item.set_code }} · {{ item.language_code }} · {{ item.region_code }} · {{ item.release_date }}</div>
        </template>
      </result-list>
    </div>
  </q-page>
