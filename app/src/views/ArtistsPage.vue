<template>
  <q-page class="bg-dark text-white q-pa-md q-pa-lg-md">
    <div class="column q-gutter-lg">
      ...
      <q-input v-model="q" dark outlined label="Search artists" @keyup.enter="load"><template #append><q-btn flat dense round icon="search" :loading="loading"
            @click="load" /></template></q-input>
      <result-list :items="results"><template #default="{ item }"><router-link class="text-primary text-subtitle1"
            :to="`/artists/${encodeURIComponent(String(item.name ?? ''))}`">{{
...
<script setup lang="ts">
  ...
  async function load() {
    loading.value = true;
    try { results.value = (await apiGet<{ results: ApiRecord[]; }>('/api/artists', { q: q.value, limit: 10 })).results ?? []; } finally { loading.value = false; }
  }
  onMounted(load);
</script>
