...
<template>
  <q-page class="bg-dark text-white q-pa-md q-pa-lg-md">
    ...
    <div class="column q-gutter-lg">
      ...
      <result-list :items="languages"><template #default="{ item }"><router-link class="text-primary text-subtitle1" :to="`/languages/${item.code}`">{{ item.name }}</router-link><div class="text-caption text-grey-5">{{ item.code }} · {{ item.status }} · {{ item.set_count }} sets · {{ item.print_count }} prints</div></template></result-list>
    </div>
  </q-page>
</template>
