...
cardPrintsRouter.get(
  ...
  asyncHandler(async (request, response) => {
    const id = parseRequiredUuid(request.params.id, 'card print id');
    ...
    const [variants, images, otherPrints, sources] = await Promise.all([
      query('SELECT * FROM print_variants WHERE card_print_id = $1 ORDER BY variant_type', [id]),
      query('SELECT ci.* FROM card_images ci JOIN print_variants pv ON pv.id = ci.print_variant_id WHERE pv.card_print_id = $1', [id]),
      query('SELECT * FROM card_prints WHERE card_concept_id = $1 AND id <> $2 ORDER BY release_date NULLS LAST', [cardPrint.card_concept_id, id]),
      query("SELECT sm.*, src.name AS source_name FROM source_mappings sm JOIN sources src ON src.id = sm.source_id WHERE sm.entity_type = 'card_print' AND sm.entity_id = $1", [id])
    ]);
