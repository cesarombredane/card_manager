<template>
  <q-page class="bg-dark text-white q-pa-md q-pa-lg-md">
    <div class="column q-gutter-lg">
      ...
      <result-list :items="results"><template #default="{ item }"><router-link class="text-primary text-subtitle1" :to="`/pokemon/${encodeURIComponent(item.name)}`">{{
...
<script setup>
  ...
  async function load() {
    loading.value = true;
    try { results.value = (await apiGet('/api/pokemon', { q: q.value, limit: 10 })).results ?? []; } finally { loading.value = false; }
  }
